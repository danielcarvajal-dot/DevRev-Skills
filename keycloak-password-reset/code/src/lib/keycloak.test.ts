...
describe('KeycloakClient', () => {
  ...
  it('re-enables a permanent lockout even after the brute-force counter was cleared', async () => {
    const http = createHttp();
    http.post.mockResolvedValue({ status: 200, data: { access_token: 'token-1', expires_in: 300 } });
    http.get.mockImplementation(async (url: string) => {
      if (url.endsWith('/users')) {
    ...
    const client = new KeycloakClient(config, http);
    const result = await client.recoverAccount('demo.user@example.com', { action: 'unlock' });
  ...
  it('records a reset-email failure without failing the unlock', async () => {
    ...
    http.put.mockImplementation(async (url: string) => {
      if (url.includes('execute-actions-email')) {
        throw { response: { status: 500, data: { errorMessage: 'smtp not configured' } } };
      }
